...
mod tests {
    ...
    impl fs::BlockDevice for RamDisk {
    ...
    fn make_fs() -> Box<dyn Files> {
        Box::new(Filesystem::format(RamDisk::new(512)).unwrap())
    }

    // ── Serialization round-trip tests ──────────────────────────────

    #[test]
    fn serialize_empty_catalog() {
        let catalog = BTreeMap::new();
    ...
    // ── Store init / open tests ─────────────────────────────────────

    #[test]
    fn store_init_and_open() {
        let fs = make_fs();
    ...
    #[test]
    fn store_create_and_media_type() {
        let fs = make_fs();
    ...
    #[test]
    fn store_delete_removes_from_catalog() {
        let fs = make_fs();
        let mut store = Store::init(fs).unwrap();
        let file = store.create("text/plain").unwrap();
    ...
    #[test]
    fn store_delete_not_in_catalog_fails() {
        let fs = make_fs();
        let mut store = Store::init(fs).unwrap();
    ...
    #[test]
    fn store_set_and_get_attribute() {
        let fs = make_fs();
        let mut store = Store::init(fs).unwrap();
        let file = store.create("text/plain").unwrap();
    ...
    #[test]
    fn store_attribute_missing_key() {
        let fs = make_fs();
        let mut store = Store::init(fs).unwrap();
        let file = store.create("text/plain").unwrap();
    ...
    fn store_metadata() {
        ...
        assert_eq!(meta.attributes.get("width").unwrap(), "100");
    }

    // ── Query tests ─────────────────────────────────────────────────

    #[test]
    fn query_media_type_exact() {
        let fs = make_fs();
        let mut store = Store::init(fs).unwrap();
    ...
    // ── Commit and reopen tests ─────────────────────────────────────

    #[test]
    fn store_commit_and_reopen() {
        let fs = make_fs();
        let mut store = Store::init(fs).unwrap();
        let file = store.create("text/plain").unwrap();
