...
use scene::{
    fnv1a, Border, Color, Content, DataRef, FillRule, NodeFlags, ShapedGlyph, TripleWriter,
...
impl SceneState {
    ...
    pub fn build_editor_scene(
...

        {
            ...
            // Test image: 32×32 BGRA gradient.
...
fn generate_test_star(size: f32) -> Vec<u8> {
...
fn generate_test_rounded_rect(w: f32, h: f32, r: f32) -> Vec<u8> {
    let mut cmds = Vec::new();
