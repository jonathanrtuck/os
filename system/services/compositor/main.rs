...
fn rasterize_svg_icon(
    ...
    unsafe {
        alloc::alloc::dealloc(path_ptr as *mut u8, alloc::alloc::Layout::new::<drawing::SvgPath>());
        alloc::alloc::dealloc(scratch_ptr as *mut u8, alloc::alloc::Layout::new::<drawing::SvgRasterScratch>());
    }
    result
}
...
pub extern "C" fn _start() -> ! {
    ...
    if init_ch.try_recv(&mut msg) && msg.msg_type == MSG_ICON_CONFIG {
        ...
        if icn.icon_va != 0 && icn.icon_len > 0 {
            ...
            if let Some((ptr, w, h)) = rasterize_svg_icon(svg_data, b"     parsing SVG doc icon\n", 20, 24) {
                sys::print(b"     SVG icon rasterized\n");
                unsafe { ICON_COVERAGE = ptr; ICON_W = w; ICON_H = h; }
            }
        }
    }
    ...
    let make_fb_surface = |idx: usize| -> drawing::Surface<'static> {
        ...
        drawing::Surface {
            data, width: fb_width, height: fb_height,
            stride: fb_stride, format: drawing::PixelFormat::Bgra8888,
        }
    };
    ...
    let scene_buf = unsafe {
        core::slice::from_raw_parts(scene_va as *const u8, scene::DOUBLE_SCENE_SIZE)
    };
