//! Store service — metadata-aware persistence layer over virtio-blk.
//!
//! Owns the virtio-blk device directly (maps MMIO, does DMA I/O) and
//! builds a COW filesystem + store layer on top. The store library
//! handles all document/metadata logic; this service is a thin IPC
//! translator.
//!
//! IPC loop handles:
//!   - MSG_STORE_COMMIT:   read doc buffer, write to file, commit
//!   - MSG_STORE_QUERY:    run store query, return matching file IDs
//!   - MSG_STORE_READ:     read file content to shared memory
//!   - MSG_STORE_SNAPSHOT: create snapshot of specified files
...
fn build_query(q: &StoreQuery) -> store::Query {
...
fn handle_query(
    ...
    let mut result = StoreQueryResult {
        count,
        _pad: 0,
        file_ids: [0u64; 6],
    };
    ...
    // SAFETY: StoreQueryResult is #[repr(C)] and fits in 60-byte payload.
    let reply = unsafe { ipc::Message::from_payload(MSG_STORE_QUERY_RESULT, &result) };
    ch.send(&reply);
...
fn handle_read(
    ...
    // SAFETY: StoreReadDone is #[repr(C)] and fits in 60-byte payload.
    let reply = unsafe { ipc::Message::from_payload(MSG_STORE_READ_DONE, &done) };
    ch.send(&reply);
    let _ = sys::channel_signal(signal);
}
...
pub extern "C" fn _start() -> ! {
    sys::print(b"  \xF0\x9F\x93\x84 store - starting\n");
    ...
    if !ch.try_recv(&mut msg) || msg.msg_type != MSG_STORE_CONFIG {
        sys::print(b"store: no config message\n");
        sys::exit();
    }

    let config = if let Some(Message::StoreConfig(c)) = decode(msg.msg_type, &msg.payload) {
        c
    } else {
        sys::print(b"store: bad config\n");
        sys::exit();
    };
    ...
    let page_va = sys::device_map(page_pa, PAGE_SIZE as u64).unwrap_or_else(|_| {
        sys::print(b"store: device_map failed\n");
        sys::exit();
    });
    ...
    if !ok {
        sys::print(b"store: negotiate failed\n");
        sys::exit();
    }
    ...
    let irq_handle: sys::InterruptHandle =
        sys::interrupt_register(config.irq).unwrap_or_else(|_| {
            sys::print(b"store: interrupt_register failed\n");
            sys::exit();
        });
    ...
    let vq_va = sys::dma_alloc(vq_order, &mut vq_pa).unwrap_or_else(|_| {
        sys::print(b"store: dma_alloc (vq) failed\n");
        sys::exit();
    });
    ...
    let buf_va = sys::dma_alloc(1, &mut buf_pa).unwrap_or_else(|_| {
        sys::print(b"store: dma_alloc (buf) failed\n");
        sys::exit();
    });
    ...
    let (filesystem, needs_store_init) = if has_filesystem {
        ...
        match fs::Filesystem::mount(blk) {
            Ok(f) => (f, false),
            Err(_) => {
                sys::print(b"store: mount failed\n");
        ...
        match fs::Filesystem::format(blk) {
            Ok(f) => (f, true),
            Err(_) => {
                sys::print(b"store: format failed\n");
                let _ = sys::channel_signal(init_handle);
                sys::exit();
            }
        }
    ...
    let mut store = if needs_store_init {
        ...
        match store::Store::init(fs_box) {
            Ok(s) => s,
            Err(_) => {
                sys::print(b"store: store init failed\n");
                let _ = sys::channel_signal(init_handle);
                sys::exit();
            }
        }
        ...
        match store::Store::open(fs_box) {
            Ok(s) => s,
            Err(_) => {
                sys::print(b"store: store open failed\n");
                let _ = sys::channel_signal(init_handle);
                sys::exit();
            }
        }
    ...
    sys::print(b"     store service ready\n");

    let doc_va = config.doc_va as usize;
    let doc_capacity = config.doc_capacity as usize;
    let content_va = config.content_va as usize;

    // Signal init that we're ready by sending MSG_STORE_READY message.
    let ready_msg = ipc::Message::new(MSG_STORE_READY);
    ch.send(&ready_msg);
    let _ = sys::channel_signal(init_handle);
    ...
    // We handle them here until MSG_STORE_BOOT_DONE signals the end.
    ...
    if content_va != 0 {
        ...
        loop {
            ...
            while ch.try_recv(&mut msg) {
                match msg.msg_type {
                    MSG_STORE_QUERY => {
                        if let Some(Message::StoreQuery(q)) = decode(msg.msg_type, &msg.payload) {
                            handle_query(&store, &q, &ch, init_handle);
                        }
                    }

                    MSG_STORE_READ => {
                        if let Some(Message::StoreRead(r)) = decode(msg.msg_type, &msg.payload) {
                            handle_read(&store, &r, r.target_va as usize, &ch, init_handle);
                        }
                    }
    ...
    loop {
        ...
        while core_ch.try_recv(&mut msg) {
            match msg.msg_type {
                MSG_STORE_COMMIT => {
                    if let Some(Message::StoreCommit(c)) = decode(msg.msg_type, &msg.payload) {
                        ...
                        if let Err(_) = store.write(file_id, 0, content) {
                            sys::print(b"store: commit write failed\n");
                        } else if let Err(_) = store.truncate(file_id, actual_len as u64) {
                            sys::print(b"store: commit truncate failed\n");
                        } else if let Err(_) = store.commit() {
                            sys::print(b"store: commit flush failed\n");
                        }
                    }
                }
                ...
                MSG_STORE_SNAPSHOT => {
                    if let Some(Message::StoreSnapshot(s)) = decode(msg.msg_type, &msg.payload) {
                        ...
                        let mut result = StoreSnapshotResult {
                            snapshot_id: 0,
                            status: 1,
                            _pad: 0,
                        };
                        ...
                        // SAFETY: StoreSnapshotResult is #[repr(C)] and fits in 60-byte payload.
                        let reply = unsafe {
                            ipc::Message::from_payload(MSG_STORE_SNAPSHOT_RESULT, &result)
                        };
                        core_ch.send(&reply);
                ...
                MSG_STORE_RESTORE => {
                    if let Some(Message::StoreRestore(r)) = decode(msg.msg_type, &msg.payload) {
                        let mut result = StoreRestoreResult { status: 1, _pad: 0 };
                        ...
                        // SAFETY: StoreRestoreResult is #[repr(C)] and fits in 60-byte payload.
                        let reply = unsafe {
                            ipc::Message::from_payload(MSG_STORE_RESTORE_RESULT, &result)
                        };
                        core_ch.send(&reply);
                        let _ = sys::channel_signal(core_handle);
                    }
                ...
                MSG_STORE_CREATE => {
                    if let Some(Message::StoreCreate(c)) = decode(msg.msg_type, &msg.payload) {
                        ...
                        // SAFETY: StoreCreateResult is #[repr(C)] and fits in 60-byte payload.
                        let reply =
                            unsafe { ipc::Message::from_payload(MSG_STORE_CREATE_RESULT, &result) };
                        core_ch.send(&reply);
                        let _ = sys::channel_signal(core_handle);
                    }
                }

                MSG_STORE_DELETE_SNAPSHOT => {
                    if let Some(Message::StoreDeleteSnapshot(d)) =
