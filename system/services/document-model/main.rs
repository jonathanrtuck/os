//! Document Model (A) — sole owner of the document buffer.
//!
...
//! Signals core (B+C) when the buffer changes so it can re-layout and re-render.
//!
//! # IPC channels (handle indices)
//!
//! Handle 1: A <-> editor (receives write operations from editor)
//! Handle 2: A <-> document service (persistence, snapshot, restore)
//! Handle 3: A <-> decoder (image decode requests/responses)
//! Handle 4: A <-> core (sends doc-changed notifications, receives undo/redo requests)
...
use protocol::{
    document::{
        DocCommit, DocCreate, DocCreateResult, DocDeleteSnapshot, DocQuery, DocQueryResult, DocRead,
        DocReadDone, DocRestore, DocSnapshot, DocSnapshotResult, MSG_DOC_COMMIT, MSG_DOC_CREATE,
        MSG_DOC_CREATE_RESULT, MSG_DOC_DELETE_SNAPSHOT, MSG_DOC_QUERY, MSG_DOC_QUERY_RESULT,
            ...
            protocol::document::decode(reply.msg_type, &reply.payload)
        {
            if result.status == 0 {
                ...
                for &snap in &discarded[..n] {
                    let del = DocDeleteSnapshot {
                        snapshot_id: snap,
                    };
                    // SAFETY: DocDeleteSnapshot is repr(C), fits in 60 bytes.
                    let del_msg =
                        unsafe { ipc::Message::from_payload(MSG_DOC_DELETE_SNAPSHOT, &del) };
                    fs_ch.send(&del_msg);
                }
                if n > 0 {
                    let _ = sys::channel_signal(state().fs_handle);
...
pub extern "C" fn _start() -> ! {
    sys::print(b"  \xF0\x9F\x93\x84 document-model starting\n");
    ...
    if !init_ch.try_recv(&mut msg) || msg.msg_type != MSG_DOC_MODEL_CONFIG {
        sys::print(b"document-model: no config message\n");
        sys::exit();
    }
    ...
    if config.doc_va == 0 {
        sys::print(b"document-model: bad config\n");
        sys::exit();
    }
