...
fn phase_3_bad_addresses() {
    ...
    for &addr in &bad_addrs {
        let ret = unsafe { raw_syscall2(30, addr, 1024) } as i64;
        if ret >= 0 {
    ...
    let ret = unsafe { raw_syscall2(30, buf.as_ptr() as u64, 0) } as i64;
    if ret >= 0 {
        phase_fail(b"phase 3", b"process_create with len=0 should fail");
    }

    // process_create with huge length.
    let ret = unsafe { raw_syscall2(30, buf.as_ptr() as u64, 100_000_000) } as i64;
    if ret >= 0 {
    ...
    let ret = unsafe { raw_syscall2(34, 0xFFFF_0000_0000_0000, 0x1000) } as i64;
    if ret >= 0 {
        phase_fail(b"phase 3", b"thread_create with kernel entry should fail");
    }
    let ret = unsafe { raw_syscall2(34, 0x1000, 0xFFFF_0000_0000_0000) } as i64;
    if ret >= 0 {
        phase_fail(b"phase 3", b"thread_create with kernel stack should fail");
    }
    // Unaligned stack.
    let ret = unsafe { raw_syscall2(34, 0x1000, 0x1001) } as i64;
    if ret >= 0 {
        phase_fail(
            b"phase 3",
