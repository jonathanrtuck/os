...
fn load_segment_pt_load() {
    ...
    let h = elf::parse_header(&data).unwrap();
    let seg = elf::load_segment(&data, &h, 0).unwrap().unwrap();
...
fn load_segment_skips_non_load() {
    ...
    assert!(elf::load_segment(&data, &h, 0).unwrap().is_none());
}
...
fn load_segment_out_of_bounds() {
    ...
    assert!(matches!(
        elf::load_segment(&data, &h, 0),
        Err(elf::Error::SegmentOutOfBounds)
    ));
}
...
fn segment_data_valid() {
    ...
    let slice = elf::segment_data(&data, &seg).unwrap();
