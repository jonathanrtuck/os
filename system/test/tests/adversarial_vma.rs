...
fn mass_insert_and_lookup() {
    ...
    for i in 0..count {
        ...
        assert!(
            ...
            "gap at {} must not match", gap_addr
        );
    }
}
...
fn vma_near_address_space_limit() {
    ...
    assert!(list.lookup(end - 1).is_some(), "last byte of high VMA must match");
}
...
fn reverse_order_insertion() {
    ...
    for i in 0..count {
        ...
        assert!(
            list.lookup(addr).is_some(),
            "VMA at {} must be found after reverse insertion", addr
        );
    }
}
...
fn scrambled_insertion_order() {
    ...
    for i in 0..count {
        ...
        assert!(
            list.lookup(addr).is_some(),
            "VMA at {} must be found after scrambled insertion", addr
        );
    }
}
...
fn permission_combinations() {
    ...
    let combos = [
        ...
        (false, false, false), // no permissions (guard page)
    ];

    for (i, &(r, w, x)) in combos.iter().enumerate() {
    ...
    for (i, &(r, w, x)) in combos.iter().enumerate() {
        ...
        assert_eq!(vma.readable, r, "readable mismatch at {}", addr);
