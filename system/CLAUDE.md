...
```text
system/
├── kernel/        # Microkernel (aarch64-unknown-none) — memory, scheduling, IPC, 27 syscalls
├── libraries/     # Shared userspace libraries (sys, virtio, drawing, ipc)
├── services/      # Platform services (init, compositor, drivers)
├── user/          # User programs (echo, fuzz, stress)
...
```sh
cd test && cargo test -- --test-threads=1   # 632 host-side unit tests
