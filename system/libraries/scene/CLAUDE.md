...
- `node.rs` -- `Node` type (136 bytes, one type with optional content), `NodeId` (u16), `SceneHeader`, millipoint coordinate types (`Mpt`/`Umpt`, 1/1024 pt), memory layout constants (`MAX_NODES` = 512, `DATA_BUFFER_SIZE` = 64 KiB, `SCENE_SIZE`)
- `primitives.rs` -- `Color`, `Border`, `Content` enum (None, InlineImage, Image, Path, Glyphs), `DataRef`, `ShapedGlyph` (16 bytes with 16.16 fixed-point advances), `FillRule`, path command encoding/decoding, `bitflags` macro, content hashing
...
- Nodes have: position, size, background, border, corner radius, opacity, clip, content, transform, children (first_child/next_sibling linked list)
