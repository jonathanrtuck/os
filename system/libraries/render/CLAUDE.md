...
CPU render backend: scene graph tree walk, content rendering, compositing, and offscreen buffer management. Used by `cpu-render` driver and shared by other render services for common logic. Pure rendering library with no dependency on `sys` or `ipc`. `no_std` with `alloc`.

## Key Files

- `lib.rs` -- `RenderBackend` trait, `CpuBackend` (full software renderer), `LruRasterizer` (on-demand glyph rasterization with LRU cache), coordinate scaling helpers
...
- `scene_render/path_raster.rs` -- Rasterize path commands to coverage maps for vector content and clip masks
...
- `frame_scheduler.rs` -- Configurable-cadence frame scheduler: event coalescing, idle optimization, frame budgeting, idle-to-active immediate wakeup
