...
pub fn run() {
    ...
    let (err, snap_h) = crate::syscall::dispatch(
        current,
        space_id,
        0,
        num::VMO_SNAPSHOT,
    ...
    let (err, evt_h) =
        crate::syscall::dispatch(current, space_id, 0, num::EVENT_CREATE, &[0, 0, 0, 0, 0, 0]);
    ...
    let (err, ep_h) = crate::syscall::dispatch(
        current,
        space_id,
        0,
        num::ENDPOINT_CREATE,
    ...
    // Clock + system_info
    let (err, _) =
        crate::syscall::dispatch(current, space_id, 0, num::CLOCK_READ, &[0, 0, 0, 0, 0, 0]);

    assert!(err == 0, "POST: clock_read failed (err={err})");

    let (err, _) =
        crate::syscall::dispatch(current, space_id, 0, num::SYSTEM_INFO, &[0, 0, 0, 0, 0, 0]);
