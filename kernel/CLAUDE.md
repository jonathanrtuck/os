...
```sh
cargo build                                           # Cross-compile for aarch64-unknown-none
cargo clippy                                          # Must be zero warnings
cd kernel/host && cargo test -- --test-threads=1      # 1,216 kernel-only tests
```
...
- Tests live in `host/tests/` and `kernel/host/tests/`, not in-file — kernel can't link `#[test]` harness on `aarch64-unknown-none`
