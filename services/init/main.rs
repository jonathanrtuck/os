...
// Service pack format constants (magic, version, role IDs).
mod pack_format {
    #![allow(dead_code)]
...
fn pack_elf(role_id: u32) -> &'static [u8] {
    ...
    if magic != pack_format::PACK_MAGIC || version != pack_format::PACK_VERSION {
        sys::print(b"FATAL: bad service pack header\n");
        sys::exit();
    }

    let entries_base = unsafe { base.add(pack_format::PACK_HEADER_SIZE as usize) };
    let entry_size = pack_format::PACK_ENTRY_SIZE as usize;
    for i in 0..count as usize {
        let entry = unsafe { entries_base.add(i * entry_size) };
        let entry_role = unsafe { core::ptr::read(entry as *const u32) };
        if entry_role == role_id {
            let offset = unsafe { core::ptr::read(entry.add(4) as *const u32) };
            let length = unsafe { core::ptr::read(entry.add(8) as *const u32) };
            return unsafe {
                core::slice::from_raw_parts(base.add(offset as usize), length as usize)
            };
        }
    }

    sys::print(b"FATAL: service not found in pack\n");
    sys::exit();
}
...
fn parse_device_manifest(next_channel: &mut usize) -> DeviceManifest {
    ...
    for i in 0..actual as usize {
        ...
        let elf: &[u8] = match dev_id {
            VIRTIO_DEVICE_BLK => pack_elf(pack_format::ROLE_STORE),
            VIRTIO_DEVICE_CONSOLE => pack_elf(pack_format::ROLE_VIRTIO_CONSOLE),
            VIRTIO_DEVICE_GPU => {
                sys::print(b"     skipping legacy virtio-gpu (no driver)\n");
                continue;
            }
            VIRTIO_DEVICE_METAL => pack_elf(pack_format::ROLE_METAL_RENDER),
            VIRTIO_DEVICE_INPUT => pack_elf(pack_format::ROLE_VIRTIO_INPUT),
            VIRTIO_DEVICE_9P => pack_elf(pack_format::ROLE_VIRTIO_9P),
            _ => {
                let mut buf = [0u8; 48];
...
fn spawn_view_engine(
    ...
    let (core_proc, _core_ch_handle, core_channel_idx) =
        match spawn_with_channel(pack_elf(pack_format::ROLE_PRESENTER), next_channel) {
            Some(v) => v,
            None => {
...
fn spawn_document_model(
    ...
    let (docmodel_proc, _docmodel_ch_handle, docmodel_ch_idx) =
        match spawn_with_channel(pack_elf(pack_format::ROLE_DOCUMENT), next_channel) {
            Some(v) => v,
            None => {
...
fn spawn_layout_engine(
    ...
    let (layout_proc, _layout_ch_handle, layout_ch_idx) =
        match spawn_with_channel(pack_elf(pack_format::ROLE_LAYOUT), next_channel) {
            Some(v) => v,
            None => {
...
fn spawn_editor(doc_pa: u64, next_channel: &mut usize) -> (sys::ProcessHandle, usize) {
    ...
    let (editor_proc, _editor_ch, editor_ch_idx) =
        match spawn_with_channel(pack_elf(pack_format::ROLE_RICH_EDITOR), next_channel) {
            Some(v) => v,
            None => {
...
fn wire_service_channels(
    ...
    *next_channel += 1;
    ...
    let decoder_proc = if content.content_pages > 0 && content.file_store_pages > 0 {
        ...
        match spawn_with_channel(pack_elf(pack_format::ROLE_PNG_DECODE), next_channel) {
...
pub extern "C" fn _start() -> ! {
    ...
    if manifest.gpu.is_none() {
        ...
        let fuzz_elf = pack_elf(pack_format::ROLE_FUZZ);
        ...
        let stress_elf = pack_elf(pack_format::ROLE_STRESS);
