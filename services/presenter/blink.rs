...
pub(crate) fn advance_blink(state: &mut ViewState, now_ms: u64) -> bool {
    let elapsed = now_ms.saturating_sub(state.cursor.blink_phase_start_ms);
    let mut changed = false;

    match state.cursor.blink_phase {
        BlinkPhase::VisibleHold => {
            ...
            if elapsed >= BLINK_VISIBLE_MS {
                ...
                state.cursor.blink_phase_start_ms = now_ms;
                changed = true;
            }
        }
        BlinkPhase::FadeOut => {
            if let Some(id) = state.cursor.blink_id {
                let new_opacity = if state.animation.timeline.is_active(id) {
                    state.animation.timeline.value(id) as u8
                } else {
                    0
                };
                if new_opacity != state.cursor.opacity {
                    state.cursor.opacity = new_opacity;
                    changed = true;
                }
            }
            if elapsed >= BLINK_FADE_OUT_MS {
                state.cursor.blink_phase = BlinkPhase::HiddenHold;
                state.cursor.blink_phase_start_ms = now_ms;
        ...
        BlinkPhase::FadeIn => {
            ...
            if elapsed >= BLINK_FADE_IN_MS {
                state.cursor.blink_phase = BlinkPhase::VisibleHold;
                state.cursor.blink_phase_start_ms = now_ms;
                state.cursor.opacity = 255;
                changed = true;
            }
...
pub(crate) fn reset_blink(state: &mut ViewState, now_ms: u64) {
    if let Some(id) = state.cursor.blink_id {
        state.animation.timeline.cancel(id);
    }
    state.cursor.blink_phase = BlinkPhase::VisibleHold;
    state.cursor.blink_phase_start_ms = now_ms;
    state.cursor.opacity = 255;
