...
pub extern "C" fn _start() -> ! {
    ...
    if ch.try_recv(&mut msg) && msg.msg_type == protocol::store::MSG_FS_CONFIG {
        if let Some(protocol::store::FsMessage::FsConfig(cfg)) =
            protocol::store::decode_fs(msg.msg_type, &msg.payload)
        {
    ...
    loop {
        ...
        while core_ch.try_recv(&mut msg) {
            if msg.msg_type == protocol::store::MSG_FS_COMMIT {
