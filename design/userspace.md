...
**No restrictions imposed.** Pure `no_std` library with no syscalls, no allocations. Callers provide the buffer. ~1584 lines, host-side tests in `host/`. The scene library is purely geometric — no content-aware code (no monospace assumptions, no line breaking, no character encoding knowledge). Content-aware layout helpers (`layout_mono_lines`, `byte_to_line_col`, `scroll_runs`) live in core where they belong.
...
**No restrictions imposed.** Pure `no_std` library with `alloc` dependency. Depends on drawing, scene, fonts, protocol. Host-side tests in `host/tests/render_scene_render.rs`.
...
**Status:** ~1300 lines. Reads device manifest, spawns drivers, orchestrates display pipeline.
...
**Status:** Owns the virtio-blk device. Formats the disk on first boot, mounts the filesystem, and runs an IPC commit loop with core. Uses the `fs` library (`libraries/fs/`) — a `no_std` port of the host prototype.
...
**Status:** ~450 lines. Implements 6 of ~30 9P2000.L operations (Tversion, Tattach, Twalk, Tlopen, Tread, Tclunk). Reads files from a shared host directory (`assets/`) via virtio transport. Currently used to load three fonts (JetBrains Mono, Inter, Source Serif 4) and a PNG image at boot.
