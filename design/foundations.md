...
**Manifest** — the file that defines a document. Every manifest has the same
shape: metadata (title, tags, custom attributes), an optional layout, and a
children list of URI references. A simple document has one child and no layout.
A compound document has multiple children with display axes (width, height,
depth, time) and a positioning mode (flow, grid, absolute) that together
determine how children are arranged. Children are URI references (`store:` for
local documents, standard schemes for external). Manifests are **one level
deep** — a child that needs internal structure is a subdocument with its own
manifest; depth comes from nesting, not inline definitions. The manifest IS the
document's identity — creating a manifest creates a document. Manifests are the
only files the metadata query system indexes. A manifest can be **static** (a
real file on disk, content written and read back) or **virtual** (a file in the
filesystem namespace whose content is generated by the OS service on read, like
Plan 9's `/proc`). Whether a manifest is static or virtual is an implementation
detail — the document interface is the same either way.
...
- How many content references the manifest has (one vs many)
- Which display axes and positioning mode are declared (a text file has none; a
  photo album has `[width, height]` grid; a slide deck has `[height]` grid of
  absolute-layout slides)
...
Documents are manifests with content references + layout descriptions. A
document with one content reference (a single text file) is structurally the
same as a document with many (a slide deck) — the manifest model is uniform.
What makes a document "compound" is not a separate concept but simply having
multiple content references with a layout that arranges them.

### Axes × Positioning Model

Layout is decomposed into two orthogonal properties: which **display axes** the
layout operates on, and how children are **positioned** along them. The
combination spans the full design space for compound documents.

**Display axes.** Four axes describe the user's perceptual space. This is a
closed set — content-domain concepts (frequency, pitch, price) are mapped to
display axes by viewers, not declared in the manifest.
...
Common combinations: `[width, height]` (article, canvas, photo grid), `[width]`
(horizontal list, carousel), `[height]` (vertical list, feed), `[time, height]`
(video timeline, DAW), `[time]` (playlist, audio sequence),
`[width, height, depth]` (3D scene).

**Axis order is direction.** The primary flow axis is the first declared axis.
`[width, height]` flows horizontally, wraps vertically. `[height, width]` flows
vertically, wraps horizontally. No separate direction property.

**Positioning modes.** Three modes, distinguished by who determines child
positions:

| Mode       | Who decides   | Description                                                      |
| ---------- | ------------- | ---------------------------------------------------------------- |
...
The two properties span the full design space:
...
New layout types are points in this space, not additions to an enum. A slide
deck is `[height]` grid (the deck) with `[width, height]` absolute (each slide).
A video editor is `[time, height]` absolute. A music playlist is `[time]` grid.

**Navigation is a viewer concern.** The manifest describes structure and spatial
layout — which axes and positioning mode apply. Whether to page through children
(slides), scroll them, or show them all at once is the viewer's decision. The
same manifest (a grid of images) can be viewed as a scrollable grid, a carousel,
or a slideshow. The desktop IS a compound document — the workspace viewer shows
one child at a time with slide animation, but the manifest is just a `[width]`
grid of documents.

**Version history is orthogonal.** COW snapshots (Decision #12) provide version
history for all documents regardless of their layout. An audio file has content
temporality (the waveform) AND version history (the edits) — fundamentally
different. Content temporality is part of what the document _is_. Version
history is how the document has _changed_. Undo operates on the version axis,
outside the layout model entirely.

### Structure

A document is a **manifest** file. Every manifest has the same shape: metadata,
an optional layout, and a children list.

- **Children** — URI references to content files. `store:` scheme for local
  documents (resolves to FileId in the document store), standard schemes
  (`https:`, `rtsp:`, `file:`) for external resources. A simple document has
  exactly one child; a compound document has many.
- **Layout** — display axes + positioning mode + per-mode properties (gap,
  alignment, divisions, bounds). Absent for simple documents.
- **Metadata** — title, tags, provenance (original source for imports), and open
  key-value attributes. Co-located with document identity.
- **Edge data** — each child carries optional placement (position/cell/span,
  dependent on positioning mode) and an optional viewport (crop/zoom into child
  content).

Manifests are **one level deep**: a flat list of children. A child that needs
internal structure is a subdocument with its own manifest. Depth comes from
subdocument nesting, not inline group definitions. The manifest IS the root node
...
manifest's placement data), not an image operation — the image bytes don't
change.
...
display axes are active:

- Resize an image → text reflows around it (flow positioning responds to changed
  intrinsic size)
- Remove a time range in a video → corresponding audio is trimmed (absolute
  positioning on the time axis enforces track synchronization)
- Reorder slides → sequence updates (grid re-indexes)
- Add a child to a flow layout → subsequent children shift
...
- Import .pptx → extract content as individual files, generate manifest with
  `[height]` grid of `[width, height]` absolute slides
