...
**From-scratch kernel (tentative).** Currently building a bare-metal aarch64 kernel. Continuing down this path to encounter real problems before evaluating alternatives (seL4, Zircon, Linux-as-runtime). The spike must hit genuine obstacles to properly appreciate what existing kernels provide. Revisitable at any time — the knowledge transfers regardless.
