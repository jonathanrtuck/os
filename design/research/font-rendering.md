...
**Font library** (`user/libraries/fonts/`): Rasterizer (`src/rasterize/`) with
...
**Scene graph** (`user/libraries/scene/lib.rs`): TextRun carries `ShapedGlyph`
arrays via DataRef in shared memory. Core writes shaped glyph data; render
services read and rasterize. Cross-process safe with compile-time size
assertion.

**Text layout** (`user/libraries/layout/`): Unified `layout_paragraph()` for
both monospace and proportional text, parameterized by `FontMetrics` trait.
...
- `user/libraries/shaping/` — new Cargo crate with HarfRust dependency
- `user/libraries/shaping/src/rasterize.rs` — glyph rasterizer using read-fonts
...
`blend_over` in `user/libraries/drawing/lib.rs` already blend in linear light
space with sRGB↔linear LUTs. The research doc was stale on this point —
...
- LRU glyph cache with configurable capacity, keyed by (glyph_id, font_size,
  ...
  differentiation.
- Font fallback chain (`user/libraries/shaping/src/fallback.rs`): ordered list
  of font data references. When primary font produces .notdef, subsequent fonts
  are tried. Content-type-aware: Code → monospace primary + proportional
...
`user/libraries/shaping/src/typography.rs` maps content types to font family,
