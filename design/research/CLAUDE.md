...
- `content-service-architecture.md` -- **Content service model.** Per-mimetype
  content handlers, document manager orchestration, portal-based scene graph
  composition, layout negotiation, event routing. Generalizes the rendering
  pipeline from the monolithic presenter to recursive compound documents. Draft.
