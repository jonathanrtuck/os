...
**Session 2026-03-22 (latest):** Phase 4 — Font rendering quality sprint. Three architectural changes to match macOS Core Text: (1) **Outline dilation (stem darkening):** Replaced coverage-space `STEM_DARKENING_LUT` with symmetric miter-join outline modification. macOS formula (Pathfinder coefficients × 1.3 empirical boost), scale-factor-aware (computes in device pixels, converts to glyph space). FreeType's `EmboldenXY` was studied but rejected — it's asymmetric (grows right/up), wrong for render-time dilation. (2) **Analytic area coverage rasterizer:** Replaced 8x vertical oversampling with exact signed-area trapezoid computation (FreeType/stb\_truetype model). Coverage is continuous, not quantized to 1/8 levels. Eliminates `OVERSAMPLE_Y`, `fill_coverage_span`, `ActiveEdge`. (3) **Device-pixel rasterization:** Metal-render now rasterizes glyphs at `font_size_pt × scale_factor` (36px at 2x Retina). Glyph atlas stores device-resolution bitmaps; quad positions scaled back to points. This was the single biggest quality improvement — eliminated 2×2 pixel-block softness. Also: `cursor_px` → `cursor_pt` naming fix in core, macOS reference screenshot (Swift script at `/tmp/macos-font-reference.png`), new `embolden.rs` module with 14 tests, new `analytic_raster.rs` with 6 tests. 2,104 tests pass. Visual result: rendering closely matches macOS Core Text at 18pt JetBrains Mono. **Next:** Continue Phase 4 visual polish — spacing, colors, effects. Or v0.3 complete.
