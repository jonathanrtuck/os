...
**Session 2026-03-22 (latest):** Phase 4 — Font rendering quality sprint. Five changes to match macOS Core Text quality: (1) **Outline dilation (stem darkening):** Replaced coverage-space `STEM_DARKENING_LUT` with symmetric miter-join outline modification. macOS formula (Pathfinder coefficients × 1.3 empirical boost), scale-factor-aware (computes in device pixels, converts to glyph space). FreeType's `EmboldenXY` studied but rejected — asymmetric (grows right/up), wrong for render-time dilation. (2) **Analytic area coverage rasterizer:** Replaced 8x vertical oversampling with exact signed-area trapezoid computation (FreeType/stb\_truetype model). Coverage is continuous, not quantized to 1/8 levels. (3) **Device-pixel rasterization:** Metal-render rasterizes glyphs at `font_size_pt × scale_factor` (36px at 2x Retina). Glyph atlas stores device-resolution bitmaps; quad positions scaled back to points. Single biggest quality improvement. (4) **Subpixel glyph positioning:** `scene::ShapedGlyph` widened from 8→16 bytes with 16.16 fixed-point i32 advances/offsets (was truncated i16). Preserves fractional advances from HarfBuzz shaping through to the renderer. DATA_BUFFER_SIZE increased 64KB→128KB. (5) **Single source of truth for char width:** Eliminated integer `char_w` — all cursor positioning, selection rects, click-to-cursor, and line wrapping use `char_w_fx` (16.16 fixed-point). Prevents cursor drift from accumulated truncation error. Also: `cursor_px` → `cursor_pt` naming fix, macOS reference screenshot, new `embolden.rs` (14 tests) + `analytic_raster.rs` (6 tests). Deferred: AA transition softness tuning and italic rendering (recorded in journal). 2,104 tests pass. Visual result: rendering closely matches macOS Core Text at 18pt JetBrains Mono. **Next:** Continue Phase 4 visual polish — spacing, colors, effects. Or v0.3 complete.

**Session 2026-03-22 (earlier):** Phase 4 — Visual Polish started. (1) **Blank slate:** Palette stripped to pure black background + pure white text. Titlebar chrome removed (no background, border, or shadow). Starting point for visual design. (2) **Three-font stack:** Replaced single Recursive variable font with JetBrains Mono (mono, editor), Inter (sans, chrome/UI), Source Serif 4 (serif, prose). Three separate variable fonts loaded by init via 9p. Protocol configs carry mono/sans/serif font lengths. Typography system updated: font family selection by content type, not variable font axis values. (3) **On-demand glyph atlas (metal-render):** Pre-scans scene graph each frame for glyph IDs not in the pre-populated ASCII atlas. Rasterizes missing glyphs on-demand, packs into atlas, uploads dirty region to GPU texture via CMD_REPLACE_REGION. Fixes silent glyph drops for programming ligatures (calt) and any non-pre-populated characters. (4) **Test migration:** All test files updated from old fonts (Nunito Sans, Source Code Pro) to new fonts. Font-specific assertions updated. 2,089 tests pass.

**Session 2026-03-22 (earlier):** Phase 3.2 — Text editor key combinations. **Architecture decision:** Navigation and selection moved to core (the OS service), not the editor. The OS owns layout and provides content-type interaction primitives (cursor, selection, playhead) — Decision #8. Editors are content-type-specific input-to-write translators. (1) **Protocol:** `KeyEvent` gained `modifiers: u8` with `MOD_SHIFT/CTRL/ALT/SUPER/CAPS_LOCK` flags. (2) **Input driver:** modifier state tracking, Caps Lock (set/clear from macOS flag state), shifted ASCII (full US keyboard). (3) **Core:** Major rewrite of `process_key_event` — all arrow navigation (with sticky goal column), Cmd+Left/Right (line start/end), Cmd+Up/Down (doc start/end), Opt+Left/Right (word boundaries), Home/End, PgUp/PgDn, Shift+any navigation (selection), Cmd+A, selection-aware backspace/delete, Opt+Backspace/Delete (word delete), double-click (word select), triple-click (line select). (4) **Layout library:** `line_col_to_byte()`, `word_boundary_backward()`/`word_boundary_forward()`. (5) **Editor:** slimmed from ~410 to ~195 lines — only character insert, backspace, delete, Tab, Shift+Tab. (6) **Hypervisor:** Caps Lock support in `handleFlagsChanged`. 13 new tests. 2,091 tests pass. **v0.3 Phase 3 COMPLETE.** Next: Phase 4 (Visual Polish, collaborative).

**Session 2026-03-22 (earlier):** Unified layout library + hypervisor event script system. (1) **Layout library** (`libraries/layout/`) — single `layout_paragraph()` function for both monospace and proportional text, parameterized by `FontMetrics` trait. `CharBreaker` (character-level wrapping) and `WordBreaker` (word boundaries with whitespace trimming). 32 new tests. Core's `layout_mono_lines()` and `byte_to_line_col()` now delegate to the library via unit-width metrics (identical behavior, new algorithm). Spec updated: Phase 3.1 renamed from "Proportional Text Layout" to "Unified Text Layout." (2) **Soft-wrap reflow fix** — `update_single_line` in incremental.rs only checked if visible line COUNT changed, not whether individual lines' content shifted when the wrap boundary moved. Fix: compare each non-changed line's glyph count against the new layout; fall back to full rebuild if any differ. (3) **Hypervisor event scripts** (`--events FILE`) — line-based format for automated input injection using evdev key names. Actions: `type`, `key`, `click`, `dblclick`, `wait`, `capture`. Events injected through standard virtio-input path. Combined with `--capture`, enables deterministic visual regression testing. (4) **Hypervisor `--resolution WxH`** — fixed pixel resolution for reproducible display tests (e.g., `--resolution 400x300` gives ~37 chars per line for wrap testing). 2,078 tests pass.
...
3. **TrueType font rasterizer built and running on bare metal.** Zero-copy TTF parser (7 tables). Scanline rasterizer with 4× vertical and 6× horizontal (subpixel) oversampling. GPOS kerning. Fonts: JetBrains Mono (monospace, editor), Inter (sans-serif, chrome/UI), Source Serif 4 (serif, prose/body), loaded from host via 9p. 21 new tests (83 total).
