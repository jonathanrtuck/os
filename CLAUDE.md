...
- `design/architecture.md` — **Read first.** The system's architectural narrative: one-way pipeline, what each component understands, where responsibilities live, decision checklist
...
**Session 2026-03-17 (latest):** Virgl Task 6 COMPLETE — glyph atlas + GPU text rendering. Title, clock, and document text render via textured quads through the virgl pipeline. 1,816 total tests pass.

**Glyph atlas + text rendering (2026-03-17):** Implemented GPU-accelerated text rendering for the virgil-render driver. Four new/modified files: `atlas.rs` (glyph texture atlas with row-based packing), `scene_walk.rs` (two-batch scene walk: `QuadBatch` for backgrounds + `TexturedBatch` for glyphs), `shaders.rs` (new `GLYPH_FS` fragment shader), `protocol/virgl.rs` (textured vertex elements + sampler mipfilter fix). Pipeline: CPU rasterizes glyphs via `fonts::GlyphCache` → packs into 512×512 R8_UNORM atlas → uploads to GPU via `TRANSFER_TO_HOST_3D` → scene walk resolves `Content::Glyphs` → emits textured quads with atlas UV coords → two-pass GPU rendering (color pipeline for backgrounds, then textured pipeline with GLYPH_FS for text). Key bugs fixed: `PIPE_TEX_MIPFILTER_NONE=2` not 3 (caused EINVAL from virglrenderer sampler validation), `Box::new(LargeStruct)` stack overflow in 16 KiB user stack (GlyphCache 238 KiB, TexturedBatch 96 KiB — fixed with `alloc_zeroed` + `Box::from_raw`). QEMU visually verified.

**Virgl Tasks 1-5 (2026-03-17, earlier):** Virgil3D GPU driver (`virgil-render`) built from scratch. Scene graph backgrounds render via GPU-accelerated triangles through virgl pipeline (virglrenderer → ANGLE → Metal → Apple GPU). TGSI text shaders, virtio-gpu 3D init, VBO triangle rendering, CCW winding fix. See `project_virgl_progress.md` memory for details.
