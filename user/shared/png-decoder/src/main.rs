//! PNG decoder service — decodes PNG images to BGRA8888 pixels.
//!
...
impl Dispatch for PngDecoder {
    fn dispatch(&mut self, msg: Incoming<'_>) {
        match msg.method {
            png_decoder::DECODE => self.handle_decode(msg),
            _ => {
...
impl PngDecoder {
    fn handle_decode(&mut self, msg: Incoming<'_>) {
        if msg.payload.len() < png_decoder::DecodeRequest::SIZE || msg.handles.is_empty() {
